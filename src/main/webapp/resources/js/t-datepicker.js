...
;(function($){
    ...
    $.fn.tDatePicker = function( pr_el, options ) {
        ...
        function setThemeCheckDate(pr_title, pr_class, pr_data_utc, pr_input, pr_fm_input) {
            ...
            '</div>'+
            '<input type="hidden" class="t-input-'+pr_class+'" id="'+pr_input+'" value="'+pr_fm_input+'" name="'+pr_input+'">'
        ...
        function showThemeDate(pr_class, pr_data_utc) {
            if ( pr_data_utc !== 0 && pr_data_utc !== null ) {
                ...
                if ( settings.showDateTheme === 'dd' ) {
                    ...
                    '<span class="t-month-'+pr_class+'">'+ check_num_10(d.getMonth()+1) +'-</span>'+
            ...
            for ( var i = 0; i < toDayElement.length; i++ ) {
                ...
                toDayElement[i].onclick = function (e) {
                    ...
                    if ( $(this).parents('.t-check-in').hasClass('t-check-in') === true ) {
                        ...
                        if ( get_utc === settings.startDate && $(this).hasClass('in') === false && dataUTC[0] >= settings.startDate ) {
                    ...
                    if ( $(this).parents('.t-check-out').hasClass('t-check-out') === true ) {
                        ...
                        if ( $(this).hasClass('in') === true ) { return; }
                        // ngăn không cho click ngày t-check-in khi đang ở t-check-out và không có ngày t-check-out
                        if ( $(this).parents('.t-datepicker').find('.in').length === 0
                            && $(this).hasClass('out') === true ) { return; }
                        ...
                        function checkNumNight(pr_el, pr_date_utc) {
                            ...
                            if ( $(pr_el).parents('.t-check-in').hasClass('t-check-in') === true ) {
                                ...
                                if ( el_hover === settings.startDate ) {
                                    if ( $(pr_el).parents('.t-check-in').find('.out').length === 0
                                        && $(pr_el).parents('.t-check-in').find('.in').length === 0 ) {
